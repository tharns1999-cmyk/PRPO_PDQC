  ...
  const handleCreateSubmit = async (e, isDraft) => {
    ...
    try {
      const itemsFormatted = prItems.map(item => {
        ...
        const pUnit = prod?.purchaseUnit || prod?.unit || 'ชิ้น';
              ...
              {prItems.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] text-slate-500 pl-9 font-medium pt-0.5">
                        ...
                        {selProd ? (
                          <>
                            ...
                            <span className="font-bold text-slate-700 font-mono">
                              ...
                              {Number(selProd.conversionRate) > 1 && (
                                <span className="text-slate-400 font-normal ml-1">
                                  (≈ {((selProd.stockBalance || 0) / selProd.conversionRate).toFixed(1).replace(/\.0$/, '')} {selProd.purchaseUnit})
                                </span>
                              )}
                            </span>
                            <span className="text-slate-300">•</span>
                            <span className="text-slate-400">จุด ROP:</span>
                            <span className="font-bold text-amber-700 font-mono">{selProd.reorderPoint} {selProd.stockUnit || selProd.unit}</span>
                            <span className="text-slate-300">•</span>
                            <span className="text-slate-400">ที่เก็บ:</span>
                            <span className="font-mono text-slate-600">{selProd.location}</span>

                            {Number(selProd.conversionRate) > 1 && (
                              <>
                                <span className="text-slate-300">•</span>
                                <span className="inline-flex items-center gap-1 bg-indigo-50 text-indigo-700 font-bold px-2 py-0.5 rounded border border-indigo-200">
                                  <Sparkles className="w-3 h-3 text-amber-500 shrink-0" />
                                  <span>เข้าคลัง: {(Number(item.qty || 0) * selProd.conversionRate).toLocaleString()} {selProd.stockUnit}</span>
                                </span>
                              </>
                            )}
                          </>
                          ...
                          <span className="text-purple-700 font-semibold bg-purple-50 px-2 py-0.5 rounded border border-purple-200 inline-flex items-center gap-1">
                            ...
                            <span>สินค้านอกแคตตาล็อก (ไม่จำเป็นต้องมีรหัสสินค้าในระบบ)</span>
                          </span>
                        ) : null}
                      </div>

                      {/* Custom Item Mode Toggle Button */}
                      <button
                        type="button"
                        ...
                        className={`text-[11px] font-bold px-2.5 py-1 rounded-lg border transition-all cursor-pointer ${
                          item.isCustom 
                            ...
                            : 'bg-slate-100 text-slate-600 border-slate-200 hover:bg-purple-50 hover:text-purple-700 hover:border-purple-200'
                        }`}
                      >
                        {item.isCustom ? '← เลือกสินค้าจาก Catalog' : '+ ระบุนอกแคตตาล็อก (Custom Item)'}
                      </button>
                    </div>
                  </div>
                );
