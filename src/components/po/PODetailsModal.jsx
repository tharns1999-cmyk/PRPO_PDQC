                        ...
                        return (
                          <tr key={idx} className={`hover:bg-slate-50/60 transition-colors ${idx % 2 === 1 ? 'bg-slate-50/20' : ''}`}>
                            ...
                            <td className="px-3 py-3">
                              ...
                              {item.onlineUrl && (
                                <a href={item.onlineUrl} target="_blank" rel="noopener noreferrer"
                                  className="inline-flex items-center gap-1 mt-0.5 text-[10px] text-indigo-600 hover:text-indigo-800 font-medium">
                                  <Globe className="w-2.5 h-2.5" /> ลิงก์สินค้า
                                </a>
                              )}
                            </td>
              ...
              {selectedPO.activityLog && selectedPO.activityLog.length > 0 && (
                <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-3">
                  <span className="text-xs font-bold text-slate-600 uppercase tracking-wider flex items-center gap-1.5">
                    <History className="w-3.5 h-3.5 text-slate-400" />ประวัติการดำเนินงาน
                  </span>
                  <div className="relative pl-5 space-y-3 border-l-2 border-slate-100 ml-1.5">
                    {selectedPO.activityLog.map((log, idx) => (
                      <div key={idx} className="relative">
                        <div className="absolute -left-[22px] top-1.5 w-2.5 h-2.5 rounded-full bg-indigo-400 border-2 border-white shadow-sm" />
                        <div className="flex items-center gap-2 text-xs font-semibold text-slate-800 flex-wrap">
                          <span>{log.action}</span>
                          <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-md font-medium">{log.role}</span>
                        </div>
                        <p className="text-[11px] text-slate-400 mt-0.5">โดย: <span className="font-medium text-slate-600">{log.user}</span> • {log.timestamp}</p>
                        {log.note && <p className="text-xs text-slate-600 mt-1 bg-slate-50 px-3 py-2 rounded-lg border border-slate-200/80">{log.note}</p>}
                      </div>
                    ))}
                  </div>
                </div>
