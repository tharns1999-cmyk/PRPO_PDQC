...
export default function PRDetailsModal({ selectedPR: initialPR, currentRole, onClose, onRefresh, onSelectPO, onEditPR }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs animate-fade-in print:hidden">
      <div className="w-full max-w-5xl max-h-[92vh] flex flex-col rounded-3xl bg-white shadow-2xl border border-slate-200/80 overflow-hidden text-slate-800 animate-zoom-in">
        ...
        <div className="shrink-0 px-6 py-4 border-b border-slate-100/90 bg-white/95 backdrop-blur-md sticky top-0 z-20">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <div className="hidden sm:flex flex-col items-end px-3.5 py-1.5 rounded-xl bg-slate-900 text-white shadow-sm">
                <span className="text-[10px] font-medium text-slate-400 uppercase tracking-wider">ยอดเงินสุทธิ</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 space-y-4">
              ...
              <div className="bg-white border border-slate-200/80 rounded-2xl overflow-hidden shadow-xs">
                  ...
                  <table className="w-full text-left text-xs">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200/80 text-[11px] font-semibold text-slate-500 uppercase tracking-wider">
                        <th className="pl-4 py-3 w-8 text-center">#</th>
                        <th className="py-3 px-3">รายการ & สเปก</th>
                        <th className="text-center py-3 px-2 whitespace-nowrap">จำนวน</th>
                        <th className="text-right py-3 px-2 whitespace-nowrap">ราคา/หน่วย</th>
                        <th className="text-right pr-4 py-3 whitespace-nowrap">รวม (฿)</th>
                      </tr>
                    </thead>
                          ...
                          <tr key={idx} className={`hover:bg-slate-50/70 transition-colors ${isEditingItems ? 'bg-amber-50/20 hover:bg-amber-50/50' : ''}`}>
                            ...
                            <td className="p-3">
                              <div className="flex items-start gap-1.5">
                                {item.code && (
                                  <span className="font-mono text-[10px] bg-slate-100 text-slate-700 px-1.5 py-0.5 rounded font-semibold border border-slate-200/60 shrink-0">
                                    {item.code}
                                  </span>
                                )}
                                <div className="font-semibold text-slate-900 text-xs leading-snug break-words" title={item.name}>
                                  {item.name}
                                </div>
                              </div>
                              
                              {item.onlineUrl && (
                                <a href={item.onlineUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 mt-1 text-[10px] text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200/80 px-2 py-0.5 rounded-md transition-colors font-medium">
                              ...
                              {rate > 1 && (
                                <div className="mt-1">
                                  <span className="text-[11px] font-mono text-slate-500 bg-slate-100 px-2 py-0.5 rounded-md inline-block border border-slate-200/60">
                                    1 {pUnit} = {rate} {sUnit}
                                  </span>
                                </div>
                              )}
                            ...
                            <td className="p-3 text-center whitespace-nowrap">
                              {isEditingItems ? (
                                ...
                                <div>
                                  <span className="font-bold text-slate-900 font-mono text-xs">{Number(pQty).toLocaleString(undefined, { maximumFractionDigits: 4 })}</span>{' '}
                            ...
                            <td className="p-3 text-right whitespace-nowrap">
                              {isEditingItems ? (
                                ...
                                <span className="font-mono font-semibold text-slate-700 text-xs">฿{Number(item.price || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                              )}
                            </td>
                            <td className="p-3 text-right font-mono font-bold text-slate-900 text-xs pr-4 whitespace-nowrap tabular-nums">
                ...
                {/* Table Financial Breakdown / Clean Dark Grand Total */}
                <div className="bg-slate-900 text-white p-4 space-y-2 text-xs rounded-b-2xl">
                  {selectedPR.financials ? (
                    <>
                      <div className="flex justify-between items-center text-slate-400">
                        <span>ยอดรวมสินค้า (Subtotal):</span>
                        <span className="font-mono font-semibold text-slate-200 tabular-nums">฿{Number(selectedPR.financials.subtotal || calculatedTotal).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                      </div>
                      ...
                      <div className="pt-2 border-t border-slate-800 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-bold uppercase tracking-wider text-slate-300">ยอดรวมสุทธิ (Grand Total):</span>
                          {selectedPR.financials.vatAmount > 0 && (
                            <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-950/80 text-emerald-400 border border-emerald-800/80">รวม VAT แล้ว</span>
                          )}
                        </div>
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold uppercase tracking-wider text-slate-300">ยอดรวมประเมินทั้งสิ้น:</span>
                      <span className="text-lg font-black font-mono text-emerald-400 tabular-nums">
            ...
            <div className="lg:col-span-5 space-y-4">
              ...
              {/* Context Bento Box (Requester, Dept, Channel, Source) */}
              <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2.5 text-xs">
                  <div className="bg-slate-50/70 p-2.5 rounded-xl border border-slate-100">
                    <span className="text-slate-400 block text-[10px] font-medium">ผู้ขอซื้อ</span>
                    <span className="font-bold text-slate-900 truncate block mt-0.5">
                      {selectedPR.requestedBy}
                    </span>
                    <span className="text-slate-500 font-mono text-[11px] block">
                      แผนก {selectedPR.department}
                    </span>
                  </div>

                  <div className="bg-slate-50/70 p-2.5 rounded-xl border border-slate-100">
                    <span className="text-slate-400 block text-[10px] font-medium">วันที่ขอซื้อ</span>
                    <span className="font-bold text-slate-900 font-mono truncate block mt-0.5">
                      {selectedPR.requestedDate || selectedPR.createdAt || '-'}
                    </span>
                  </div>

                  <div className="bg-slate-50/70 p-2.5 rounded-xl border border-slate-100">
                    <span className="text-slate-400 block text-[10px] font-medium">ช่องทางจัดซื้อ</span>
                    <span className="font-bold text-slate-900 truncate block mt-0.5">
                  ...
                  <div className="bg-slate-50/70 p-2.5 rounded-xl border border-slate-100">
                    <span className="text-slate-400 block text-[10px] font-medium">สถานที่ใช้งาน</span>
                    <span className="font-bold text-slate-900 truncate block mt-0.5">
        ...
        {/* ── Consolidated Action Dock (Sticky Bottom / Non-scrollable) ── */}
        <div className="shrink-0 px-6 py-3.5 bg-white/95 backdrop-blur-md border-t border-slate-100 sticky bottom-0 z-20">
          {isRevising ? (
              ...
              {(['SUBMITTED', 'REVIEWED', 'REJECTED_TO_L2'].includes(selectedPR.status)) && 
               (workflowEngine.canAction(currentRole, selectedPR)) && (
                <div className="relative">
                  ...
                  <input 
                    type="text" 
                    placeholder="ระบุความเห็น / บันทึกกำกับการอนุมัติ (ถ้ามี)..." 
                    className="w-full bg-slate-50/80 hover:bg-white focus:bg-white border border-slate-200/90 rounded-xl pl-10 pr-4 py-2 text-xs font-medium focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 outline-none transition-all shadow-2xs placeholder-slate-400"
                    value={actionNote}
